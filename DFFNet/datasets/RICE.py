...
class RICE:
    ...
    def get_loaders(self):
        print("=> evaluating RICE set...")
       

        train_dataset = RICEDataset(dir=os.path.join(self.config.data.data_dir, 'train'), val=False, transforms=self.transforms)
        val_dataset = RICEDataset(dir=os.path.join(self.config.data.data_dir, 'val'), val=True, transforms=self.transforms)
...
class RICEDataset(torch.utils.data.Dataset):
    def __init__(self, dir, val, transforms):
        ...
        RICE_inputs = os.path.join(RICE_dir, 'hazy')
        RICE_gt = os.path.join(RICE_dir, 'clear')

        input_images = [f for f in listdir(RICE_inputs) if isfile(os.path.join(RICE_inputs, f))]
        gt_images = [f for f in listdir(RICE_gt) if isfile(os.path.join(RICE_gt, f))]

        # 直接根据文件名匹配 hazy 和 clear 图像
        for hazy in input_images:
            if hazy in gt_images:  # 确保 hazy 图像和 clear 图像一一对应
                input_names.append(os.path.join(RICE_inputs, hazy))
                gt_names.append(os.path.join(RICE_gt, hazy))  # 直接使用相同的名称
