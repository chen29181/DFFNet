...
class DHID:
    ...
    def get_loaders(self):
        ...
        train_dataset = DHIDDataset(dir=os.path.join(self.config.data.data_dir, 'train'), val=False, transforms=self.transforms)
        val_dataset = DHIDDataset(dir=os.path.join(self.config.data.data_dir, 'val'), val=True, transforms=self.transforms)
...
class DHIDDataset(torch.utils.data.Dataset):
    def __init__(self, dir, val,  transforms):
        ...
        DHID_inputs = os.path.join(DHID_dir, 'input') 


        DHID_inputs = os.path.join(DHID_dir, 'input')
        input_images = [f for f in sorted(listdir(DHID_inputs)) if isfile(os.path.join(DHID_inputs, f))]
        input_names += [os.path.join(DHID_inputs, i) for i in input_images]
        
        DHID_gt = os.path.join(DHID_dir, 'target')
        gt_images = [f for f in sorted(listdir(DHID_gt)) if isfile(os.path.join(DHID_gt, f))]
        gt_names += [os.path.join(DHID_gt, i) for i in gt_images]
