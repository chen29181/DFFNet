...
def validation(net, val_data_loader, device, config, save_tag=False):
    ...
    for i, (x, y) in enumerate(val_data_loader):
        ...
        with torch.no_grad():
            ...
            pred_image, haze_recon = net(input)
            #pred_image, haze_recon,ffb_out = net(input)
        ...
        if save_tag:
            # print()
            save_image(pred_image, y, config, "predict")
            save_image(haze_recon, y, config, "haze_recon")
            #save_image(ffb_out, y, config, "ffb_out")
...
#         filename = './results/{}/{}.jpg'.format(config.data.dataset, image_name[ind])
...
def save_image(pred_image, image_name, config, image_type):
    pred_image_images = torch.split(pred_image, 1, dim=0)
    batch_num = len(pred_image_images)
    
    for ind in range(batch_num):
        # print(image_name[ind])
        filename =  './results/{}/{}/{}.jpg'.format(config.data.dataset, image_type,image_name[ind])
        os.makedirs(os.path.dirname(filename), exist_ok=True)
        utils.save_image(pred_image_images[ind], './results/{}/{}/{}.jpg'.format(config.data.dataset, image_type,image_name[ind]))
        # utils.save_image(pred_image_images[ind], './Ablation/fusion_results/{}/fusion3/{}.jpg'.format(config.data.dataset, image_name[ind]))
